  ...
  try {
    ...
    data.forEach(row => {
      ...
      switch (row.id) {
        ...
        case 'industries': populateIndustriesCMS(row.content); break;
        case 'process': populateProcessCMS(row.content); break;
...
function populateIndustriesCMS(c) {
  const container = document.getElementById('industries-items');
  if(container) {
    container.innerHTML = '';
    (c.items || []).forEach((item, i) => addIndustryItem(item, i));
  }
}

function populateProcessCMS(c) {
  const container = document.getElementById('process-items');
  if(container) {
    container.innerHTML = '';
    (c.items || []).forEach((item, i) => addProcessItem(item, i));
  }
}
...
function addIndustryItem(data = {}, index) {
  const container = document.getElementById('industries-items');
  const idx = index !== undefined ? index : container.children.length;
  const div = document.createElement('div');
  div.className = 'item-block';
  div.innerHTML = `
    <div class="item-block-header">
      <span class="item-block-label">Industry ${idx + 1}</span>
      <button type="button" class="btn-remove-item" onclick="this.closest('.item-block').remove()">Remove</button>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
      <div class="cms-field">
        <label class="cms-label">Icon (emoji)</label>
        <input type="text" class="cms-input ind-icon" value="${escHtml(data.icon || '')}" placeholder="🏢" />
      </div>
      <div class="cms-field">
        <label class="cms-label">Title</label>
        <input type="text" class="cms-input ind-title" value="${escHtml(data.title || '')}" placeholder="Industry title" />
      </div>
    </div>
  `;
  container.appendChild(div);
...
function addProcessItem(data = {}, index) {
  const container = document.getElementById('process-items');
  const idx = index !== undefined ? index : container.children.length;
  const div = document.createElement('div');
  div.className = 'item-block';
  div.innerHTML = `
    <div class="item-block-header">
      <span class="item-block-label">Step ${idx + 1}</span>
      <button type="button" class="btn-remove-item" onclick="this.closest('.item-block').remove()">Remove</button>
    </div>
    <div class="cms-field">
      <label class="cms-label">Title</label>
      <input type="text" class="cms-input proc-title" value="${escHtml(data.title || '')}" placeholder="Step title" />
    </div>
    <div class="cms-field">
      <label class="cms-label">Description</label>
      <textarea class="cms-textarea proc-desc" style="min-height:70px;">${escHtml(data.desc || '')}</textarea>
    </div>
  `;
  container.appendChild(div);
}
...
async function saveCMS(section) {
  ...
  try {
    switch (section) {
      ...
      case 'industries':
        content = {
          items: Array.from(document.querySelectorAll('#industries-items .item-block')).map(block => ({
            icon: block.querySelector('.ind-icon').value.trim(),
            title: block.querySelector('.ind-title').value.trim(),
          })),
        };
        break;
      case 'process':
        content = {
          items: Array.from(document.querySelectorAll('#process-items .item-block')).map(block => ({
            title: block.querySelector('.proc-title').value.trim(),
            desc: block.querySelector('.proc-desc').value.trim(),
          })),
        };
        break;
      case 'faq':
...
function getProjectForm(d) {
  ...
  return `
    ...
    <div class="form-group">
      ...
      <input type="text" id="m-proj-category" class="form-input" value="${escHtml(meta.category || '')}" placeholder="Healthcare, Restaurant..." />
    </div>
    <div class="form-group">
    ...
    <label style="display:flex;align-items:center;gap:10px;cursor:pointer;margin-top:12px;">
      <label class="toggle-switch" style="width:44px;height:24px;flex-shrink:0;">
        <input type="checkbox" id="m-proj-featured" ${meta.is_featured ? 'checked' : ''} />
        <span class="toggle-slider"></span>
      </label>
      <span style="font-size:0.88rem;color:var(--text-muted);">Featured Case Study (Shows large on Homepage)</span>
    </label>
  `;
}
  ...
  try {
    if (modalMode === 'project') {
      ...
      if (editingId) {
        ...
        projectMeta[editingId] = {
          category: document.getElementById('m-proj-category').value.trim(),
          is_featured: document.getElementById('m-proj-featured').checked
        };
        await supabaseClient.from('cms_content').upsert({ id: 'project_meta', content: projectMeta, updated_at: new Date().toISOString() });
        toast('success', 'Project updated!');
      } else {
        const { data: newProj, error } = await supabaseClient.from('projects').insert(payload).select().single();
        if(newProj) {
          projectMeta[newProj.id] = {
            category: document.getElementById('m-proj-category').value.trim(),
            is_featured: document.getElementById('m-proj-featured').checked
          };
          await supabaseClient.from('cms_content').upsert({ id: 'project_meta', content: projectMeta, updated_at: new Date().toISOString() });
